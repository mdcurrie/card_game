...
class Player:
	...
	def draw_card(self):
		...
		if self.is_empty('deck') == False:
			...
			if self.is_empty('discard') == True:
				print('The deck and discard pile are empty. Cannot draw a card.')
				return
	...
	def display_stats(self):
		# prints the number of actions, buys, and gold a player has
		print('Actions: {}   Buys: {}   Gold: {}'.format(
			self.actions, self.buys, self.gold))
	...
	def transfer_cards(self, cards_from, cards_to):
		...
		try:
			...
			print('KeyError in transfer_cards()')
			return
		else:
	...
	def remove_all_cards(self, card_list):
		...
		try:
			...
			print('KeyError in remove_all_cards()')
			return
		else:
